<template>
	<view class="date_of_birth same_line">
		<span class="title">出生年月：</span>
		<view class="select_items">
			<view @click="openCalendar" :class="[isbrithTip ? '' : 'sel_font_style']">{{ isbrithTip && date_of_birth === '' ? '请选择出生年月' : date.year + '-' + date.month + '-' + date.day }}</view>
		</view>
	...
	export default {
		...
		mounted () {
			if (this.date_of_birth) {
				const millisecond_date = this.date_of_birth * 1000
		...
		methods: {
			...
			timeCofirm (e) {
				this.$emit('getbirthTime', this.date.year, this.date.month, this.date.day);
				this.visible = e[0]
				if (e[1] === 1) {
					this.isbrithTip = false
				}
			}
		},
		components: {

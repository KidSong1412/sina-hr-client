<template>
	<view class="appointment">
		<view class="base_info block_style">
			...
			<view class="info">投递时间：{{ getFormatDate(info.create_time)}}</view>
			<view>简历来源：{{ info.source || '待补充' }}</view>
		</view>
		<view class="toSignIn">
			<view class="first_level_title">面试预约</view>
			<view class="sign_in block_style">
				...
				<view class="toSignBtn">
					<button style="background: #169bd4; color: #ffffff;" @click="toAppointment()">确认提交</button>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			async toAppointment () {
				...
				const { message } = await Appointment.Order(id, date, userId, username)
				// const { message } = await Appointment.Order(id, date)
				if (message === '预约成功') {
					uni.showToast({
					    title: '预约成功',
					    duration: 2000
